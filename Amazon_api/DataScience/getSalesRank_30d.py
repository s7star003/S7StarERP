...
@swagger_auto_schema(
    method='get',
    operation_description="获取Amazon近30天销售排行数据",
    responses={200: openapi.Response('返回Amazon近30天销售排行数据')},
    tags=['Amazon']
)
@api_view(['GET'])
def get_sales_rank_30d(request):
    file_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
        'MultiplatformDataDashboardDataSource', 'Amazon', 'SalesRank_30d.json')
    try:
