...
@swagger_auto_schema(
    method='get',
    operation_description="获取Amazon全部区域需求分析数据",
    responses={200: openapi.Response('返回Amazon全部区域需求分析数据')},
    tags=['Amazon']
)
@api_view(['GET'])
def get_area_demand_all(request):
    file_path = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
        'MultiplatformDataDashboardDataSource', 'Amazon', 'AreaDemand_all.json')
    try:
