...
OUTPUT_DIR = os.path.join(PROJECT_ROOT, "MultiplatformDataDashboardDataSource", "Shein")
...
# 修正明细抓取：遍历每个订单的orderGoodsInfoList
order_items = []
with open(ORDER_ITEMS_FILE, "r", encoding="utf-8") as f:
    ...
    for order in raw:
        ...
        shipping_fee = order.get("sellerShippingFee", 0)
        for good in order.get("orderGoodsInfoList", []):
            good["orderNo"] = order_no
            good["sellerShippingFee"] = shipping_fee
...
for order in order_map.values():
    for item in order.get("line_items", []):
        ...
        image = item.get("spuPicURL") if "spuPicURL" in item else ""
        if sku and image:
            sku_image_map[sku] = image

def in_days(order, days):
    now = datetime.now()
    dt_raw = order.get("orderCreateTime") or order.get("orderUpdateTime")
    if not dt_raw:
        return False
    try:
...
def get_sku(order):
    if order.get("line_items") and len(order["line_items"]) > 0:
        ...
        return item.get("skuCode") or item.get("goodsSn") or item.get("sellerSku")
    return None

def get_area(order):
    return order.get("salesSite") or order.get("warehouseName")

def get_buyer(order):
    return order.get("orderNo")  # Shein无买家唯一ID，暂用订单号

def extract_amount(order, item=None):
    # 优先用明细的sellerCurrencyPrice
    if item and item.get("sellerCurrencyPrice"):
        try:
            return float(item["sellerCurrencyPrice"])
        except Exception:
            return 0
    return 0

def extract_shipping_fee(order, item=None):
    if item and item.get("sellerShippingFee"):
        try:
            return float(item["sellerShippingFee"])
        except Exception:
            return 0
    return 0
...
def is_valid_order(order):
    ...
    for item in order["line_items"]:
        if extract_amount(order, item) > 0:
            return True
    return False
            ...
            if extract_amount(order, item) > 0:
                ...
                if area and sku:
                    if days == 'all' or in_days(order, days):
                        ...
                        amount = extract_amount(order, item)
...
for sku, buyers in sku_buyer.items():
    repurchase_users = [b for b in buyers if sum(1 for o in order_map.values() for i in o.get("line_items", []) if get_buyer(o)==b and ((i.get("skuCode") or i.get("goodsSn") or i.get("sellerSku"))==sku) and extract_amount(o, i)>0) > 1]
    if len(repurchase_users) == 0:
        continue
    repurchase_rate = len(repurchase_users) / len(buyers) if buyers else 0
    area_counter = Counter()
    for b in repurchase_users:
        user_orders = [o for o in order_map.values() for i in o.get("line_items", []) if get_buyer(o)==b and ((i.get("skuCode") or i.get("goodsSn") or i.get("sellerSku"))==sku) and extract_amount(o, i)>0]
...
for order in order_map.values():
    for item in order.get("line_items", []):
        ...
        if int(item.get("goodsStatus", 0)) in RETURN_STATUS:
            sku_return[sku]["return"] += 1
            reason = "退货/取消"  # Shein明细无详细退货原因
...
for order in order_map.values():
    for item in order.get("line_items", []):
        if extract_amount(order, item) > 0:
            sku = item.get("skuCode") or item.get("goodsSn") or item.get("sellerSku")
            pay_price = extract_amount(order, item)
            shipping_fee = extract_shipping_fee(order, item)
            # 优先用平台毛利公式
            try:
                store_coupon = float(item.get("orderCurrencyStoreCouponPrice") or 0)
                promotion = float(item.get("orderCurrencyPromotionPrice") or 0)
                perf_service = float(item.get("performanceServiceCharge") or 0)
                commission = float(item.get("commission") or 0)
                platform_fee = pay_price * 0.05
                profit_formula = pay_price - store_coupon - promotion - perf_service - commission - platform_fee
                sku_profit[sku]["total_profit"] += profit_formula
                sku_profit[sku]["total_orders"] += 1
                sku_profit[sku]["profit_methods"]["platform_formula"] += profit_formula
                sku_profit[sku]["profit_explain"]["platform_formula"] = "毛利=售价-seller券-平台促销-履约服务费-佣金-平台提点5%（平台原生字段+5%平台提点）"
            except Exception:
                # 回退到自定义公式
                profit = pay_price - shipping_fee
                sku_profit[sku]["total_profit"] += profit
                sku_profit[sku]["total_orders"] += 1
                sku_profit[sku]["profit_methods"]["paid_price-sellerShippingFee"] += profit
                sku_profit[sku]["profit_explain"]["paid_price-sellerShippingFee"] = "毛利=paid_price-sellerShippingFee，仅统计有sellerCurrencyPrice的明细"
...
for order in order_map.values():
    for item in order.get("line_items", []):
        if extract_amount(order, item) > 0:
            sku = item.get("skuCode") or item.get("goodsSn") or item.get("sellerSku")
            order_date = order.get('orderCreateTime') or order.get('orderUpdateTime')
            if not order_date:
                continue
            try:
...
print("所有Shein分析结果已输出到:", OUTPUT_DIR) 